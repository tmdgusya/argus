...
2. `/setprivacy` 입력
3. 대상 봇 선택 (`@argus_dba_bot`)
4. `Disable` 선택
5. 완료 후 그룹에서 `@argus_dba_bot 안녕하세요` 테스트
...
```bash
sed -i '/^TELEGRAM_REQUIRE_MENTION=/d' ~/.hermes/profiles/dba/.env
echo 'TELEGRAM_REQUIRE_MENTION=true' >> ~/.hermes/profiles/dba/.env
dba gateway restart
```

동작:
- 그룹에서 `@argus_dba_bot ...` 멘션이 있을 때만 응답
- 멘션 없는 일반 메시지는 무시
